{
  description = ''
    Rust + Nix overlay for an EXISTING Rust project.

    Use this variant when you already have a Cargo project (Cargo.toml /
    Cargo.lock / src/) and want to add Nix on top to manage the toolchain,
    system libraries, and dev environment WITHOUT replacing or duplicating
    anything Cargo already owns.

    What this template ships (and only this):
      flake.nix           # crane build + dev shell, reads YOUR Cargo.lock
      flake.lock          # committed
      rust-toolchain.toml # single source of truth for the Rust toolchain
      .envrc              # direnv / nix-direnv auto-load
      .gitignore          # adds Nix entries; merge with your existing one

    What this template does NOT ship (because you already have them):
      Cargo.toml, Cargo.lock, src/

    If instead you want a fresh Rust project scaffold with placeholder
    Cargo.toml and src/main.rs included, use the `rust` template variant.

    Division of labor:
      * Cargo owns crate dependencies (Cargo.toml / Cargo.lock).
      * Nix owns the toolchain, system libraries, and reproducible env.

    The Rust toolchain is read from rust-toolchain.toml (single source of
    truth). Crate hashes are derived from your Cargo.lock by crane — no
    hand-maintained hashes. Build inputs and env vars are declared once and
    shared by devShell + package so `cargo build` in `nix develop` and
    `nix build` agree.
    ...
    Integration steps (after `nix flake init -t ...#rust-integrate`):
      1. Merge .gitignore entries with your existing .gitignore.
      2. If your repo already pins a Rust version (e.g. an existing
         rust-toolchain.toml or rust-toolchain file), reconcile it with the
         one shipped here — keep only one.
      3. Add native deps your crates need to commonBuildInputs in flake.nix.
      4. `nix flake check` to verify clippy / fmt / tests run under Nix.
      5. `git add` the new files (including flake.lock and rust-toolchain.toml)
         and commit.

    Quick start:
      nix develop          # enter dev shell (or `direnv allow` if using nix-direnv)
      nix build            # build YOUR existing crate via crane
    ...
    let
      ...
      perSystemOutputs =
        ...
        let
          ...
          # default features, git2, native-tls, ...) and this template lands on an
          # existing project whose deps are unknown. Drop all three if the target
          # project stays on pure-rustls.
